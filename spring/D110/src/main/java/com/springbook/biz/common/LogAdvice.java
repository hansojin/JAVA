...
/*
	...
	따라서 여러 포인트 컷을 식별하기 위한 식별자가 필요한데, 이때 참조 메소드를 이용한다.
	참조 메소드는 메소드 몸체가 비어있는, 즉 구현 로직이 없는 메소드이다. 따라서 어떤 기능처리를 목적으로 하지 않고 단순히 포인트컷을 식별하는 이름으로만 사용된다.
	
	위 소스는 allPointcut()과 getPointcut() 메소드 위에 각각 @Pointcut을 이용하여 두 개의 포인트컷을 선언했다. 
	그러면 이후에 이 포인트컷을 참조할 때, @Pointcut이 붙은 참조 메소드 이름을 이용하여 특정 포인트컷을 지정할 수 있다.
	...
	위 소슨느 allPointcut() 메소드로 지정한 포인트컷 메소드가 호출될 때, printLog()라는 어드바이스 메소드가 실행되도록 설정한 것이다. 
	그리고 이 printLog() 메소드 위에 @Before가 설정되었으므로 printLog() 메소드는 사전 처리 형태로 동작한다.
*/