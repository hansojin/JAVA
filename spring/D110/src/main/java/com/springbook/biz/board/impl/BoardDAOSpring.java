...
/*
	스프링 JDBC 개념
	JDBC를 이용하여 DB 연동 프로그램을 개발하면 데이터베이스에 비종속적인 DB 연동 로직을 구현할 수 있다. 
	BoardDAO클래스에 insertBoard()와 updateBoard() 메소드는 실행되는 SQL 구문과 ?에 설정하는 값만 다를 뿐, JDBC에 해당하는 자바 코드는 거의 같다.
	
	이런 환경에서 새로운 기능의 메소드를 개발하려면, 결국 기존 메소드를 복사하여 SQL만 수정하는 방법뿐이다. 
	그런데 만약 DB 연동에 필요한 자바 코드를 누군각가 대신 처리해주고 개발자는 실행되는 SQL 구문만 관리한다면 개발과 유지 보수는 훨씬 편해질 것이다. 
	스프링은 JDBC 기반의 DB 연동 프로그램을 쉽게 개발할 수 있도록 JdbcTemplate 클래스를 지원한다.
	
	JdbcTemplate 클래스
	JdbcTemplate은 GoF 디자인 패턴 중 템플릿 메소드 패턴이 적용된 클래스이다. 
	템플릿 메소드 패턴은 복작하고 반복되는 알고리즘을 캡슐화해서 재사용하는 패턴으로 정의할 수 있다. 
	템플릿 메소드 패턴을 이용하면 반복해서 사용도는 알고리즘을 템플릿 메소드로 캡슐화할 수 있어서 JDBC처럼 코딩 순서가 정형화된 기술에서 유용하게 사용할 수 있다. 
	따라서 반복되는 DB 연동 로직은 JdbcTemplate 클래스의 템플릿 메소드가 제공하고, 개발자는 달라지는 SQL 구문과 설정값만 신경 쓰면 된다.
	
	JdbcTemplate은 JDBC의 반복적인 코드를 제거하기 위해 제공하는 클래스다. 
	...
	JdbcTemplate 클래스가 어떻게 JDBC API를 이용하는지 DAO 클래스 개발자는 전형 신경 쓸 필요가 없다.
	...
	2. queryForInt() 메소드
	SELECT 구문으로 검색된 정숫값을 리턴받으려면 queryForInt() 메소드를 사용한다. 매개변수의 의미는 앞에서 살펴본 update() 메소드와 같다.
	...
	queryForObject() 메소드는 SELECT 구문의 실행 결과를 특정 자바 객체(Value Object)로 매핑하여 리턴받을 때 사용한다. 
	queryForObject() 메소드는 검색 결과가 없거나 검색 결과가 두 개 이상이면 예외(IncorrectResultSizeDataAccessException)를 발생시킨다.
	...
	기본 사용법은 queryForObject() 메소드와 같다. 따라서 query() 메소드에서도 검색 결과를 VO 객체에 매핑하려면 RowMapper 객체를 사용한다.
	...
	query() 메소드가 실행되면 여러 건의 ROW 정보가 검색되며, 검색된 데이터 ROW 수만큼 RowMapper 객체의 mapRow() 메소드가 실행된다. 
	...
	그리고 getJdbcTemplate() 메소드를 호출하면 JdbcTemplate 객체가 리턴되어 모든 메소드를 JdbcTemplate 객체로 구현할 수 있다.
	그런데 문제는 getJdbcTemplate() 메소드가 JdbcTemplate 객체를 리턴하려면 DataSource 객체를 가지고 있어야 한다. 
	따라서 반드시 부모 클래스인 JdbcDaoSupport 클래스의 setDataSource() 메소드를 호출하여 데이터소스 객체를 의존성 주입해야 한다.
	...
	@Autowired 어노테이션은 주로 변수 위에 선언하는데 메소드 우에 선언해도 동작한다. 
	메소드 위에 @Autowired를 붙이면 해당 메소드를 스프링 컨테이너가 자동으로 호출해주며, 
	이때 메소드 매개변수 타입을 확인하고 해당 타입의 객체가 메모리에 존재하면 그 객체를 인자로 넘겨준다.
	수정된 모든 파일들을 저장하고 BoardServiceClient 프로그램을 실행하여 결과를 확인한다.
	
	두 번째 방법 : JdbcTemplate 클래스 <bean> 등록, 의존성 주입
	
	DAO 클래스에서 JdbcTemplate 객체를 얻는 두 번째 방법은 JdbcTemplate 클래스를 <bean> 등록하고, 의존성 주입으로 처리하는 것이다. 
	일반적으로 이 방법을 사용한다. 먼저 스프링 설정 파일에 JdbcTemplate 클래스를 <bean> 등록한다.
	...
	이때 반드시 JdbcTemplate 객체에 DataSource 객체를 의존성 주입해야 한다. 
	...
	수정된  소스를 모두 저장하고 다시 한 번 BoardServiceClient 프로그램을 실행해보자. 
	기존에 JDBC 기반으로 동작했던 BoardDAO가 아닌 스프링 JDBC 기반의 BoardDAOSpring 으로 DB 연동이 처리된다는 점만 다르고 실행 결과는 같다.
*/