...
/*	
	...
	하지만 JdbcTemplate 클래스가 어떻게 JDBC API를 이용하는지 DAO 클래스 개발자는 전형 신경 쓸 필요가 없다.
	
	JdbcTemplate 메소드
	
	- update() 메소드
	
	"INSERT, UPDATE, DELETE 구문을 처리"하려면 JdbcTemplate 클래스의 update() 메소드를 사용한다. 
	...
	public void updateBoard(BoardVO vo) {
		...
		int cnt = jdbcTemplate.update(SQL_UPDATE, vo.getTitle(), vo.getContent(), vo.getSeq());
	...
	매개변수의 의미는 앞에서 살펴본 update() 메소드와 같다.
	...
	public int getBoardTotalCount(BoardVO vo){
		String SQL_TOT_COUNT="select count(*) from board";
		int count = jdbcTemplate.queryForInt(SQL_TOT_COUNT);
	...
	그리고 중요한 것은 "검색 결과를 자바 객체(Value Object)로 매핑할 RowMapper 객체를 반드시 지정"해야 한다.
	...
	query() 메소드는 "SELECT 문의 실행 결과가 목록일 때" 사용한다. 기본 사용법은 queryForObject() 메소드와 같다. 
	따라서 query() 메소드에서도 검색 결과를 VO 객체에 매핑하려면 "RowMapper 객체를 사용"한다.
	...
	사용 예
	public List(BoardVO> getBoardList(BoardVO vo) {
		String SQL_BOARD_LIST = "select * from board order by seq desc";
		return jdbcTemplate.query(SQL_BOARD_LIST, new BoardRowMapper());
	}
	...
	의존성 주입으로 처리하는 것이다. 일반적으로 이 방법을 사용한다. 먼저 스프링 설정 파일에 JdbcTemplate 클래스를 <bean> 등록한다.
	...
	JdbcTemplate 객체를 이용하여 BoardDAOSpring 클래스를 구형했으면 이제 BoardServiceImple 클래스가 
	BoardDAOSpring 객체를 이용하여 DB 연동을 처리하도록 수정하자. 앞에서 작성했던 BoardServiceImple 클래스에서 
	boardDAO 변수의 타입을 BoardDAO에서 BoardDAOSpring으로만 수정하면 된다.
