...
public class BoardController {
	...
	/*
		@RequestParam에서 사용된 속성의 의미
		value :  화면으로부터 전달될 파라미터 이름
		defaultValue : 화면으로부터 전달될 파라미터 정보가 없을 때, 설정할 기본값
		required : 파라미터의 생략 여부
		
		결국 위 설정은 searchCondition이 필수 파라미터는 아니지만, 
		만약 파라미터 정보가 전달된다면 해당 값을 추출하여 condition 변수에 할당하고, 
		...
		물론 @RequestParam을 사용하기 싫다면,
		BoardVO 클래스에 searchCondition, searchKeyword 변수를 추가하고 Getter/Setter 메소드만 생성하면 간단하게 처리할 수도 있다.
