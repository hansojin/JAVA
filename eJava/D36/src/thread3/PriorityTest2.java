...
class MesssageSendingThread extends Thread{
	...
	public MesssageSendingThread(String str, int prio) {
		Messsage=str;
		setPriority(prio);
	}
	public void run(){
		for(int i=0; i<1000; i++)
			System.out.println(Messsage+"("+getPriority()+")");
	}	
}

class PriorityTest2{
	public static void main(String[] args){
		MesssageSendingThread tr1
			=new MesssageSendingThread("First", Thread.MAX_PRIORITY);
		MesssageSendingThread tr2
			=new MesssageSendingThread("Second", Thread.NORM_PRIORITY);
		MesssageSendingThread tr3
			=new MesssageSendingThread("Third", Thread.MIN_PRIORITY);
