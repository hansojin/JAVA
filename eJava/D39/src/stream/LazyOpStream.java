...
// peek 와 forEach의 역할이 같음
// peek은 최종 연산이 있는 경우에 사용 가능
// forEach는 for 최종 연산 (return type : void)
...
즉 위의 메소드들은 '최종 연산'이냐 '중간 연산'이냐 하는 부분에서만 forEach와 차이가 있다. 
그럼 위의 예제를 통해서 스트림의 '지연(Lazy) 처리'를 확인해 보자.
...
따라서 sum은 프로그램의 실행 결과에 아무런 영향을 주지 않는다. 그럼에도 불구하고 
이 연산으로 인해 파이프라인에 데이터가 흐른다. 따라서 중간 연산이 실행된다.
