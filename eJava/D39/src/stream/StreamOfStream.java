...
class StreamOfStream {
    public static void main(String[] args) {
        ...
        System.out.println();   
        //[Toy, Robot, Box] _ list 객체 하나짜리 stream이 만들어지므로 []로 출력
...
위 예제에서는 다음 메소드 호출을 통해서 네 개의 정수로 이루어진 스트림을 생성하였다. 물론 스트림의 생성 과정에서 Integer형으로 오토 박싱이 진행된다.
...
생성된 스트림에는 "하나의 인스턴스만 존재"한다. 그리고 그 인스턴스는 "참조변수 s1이 참조하는 컬렉션 인스턴스"이다.
이렇듯 컬렉션 인스턴스도 그 자체로 스트림을 이루는 데이터가 되게 할 수 있다. 이는 언뜻 불필요할 것 같지만 복잡한 데이터를 관리하고 처리할 경우 유용하게 사용되기도 한다.

[참고] Stream.of 메소드에 "배열을 전달하는 경우"
Stream.of 메소드에 컬렉션 인스턴스를 전달하면 해당 인스턴스 하나로 이뤄진 스트림이 생성됨을 예제를 통해 보였다. 
그런데 Stream.of 메소드에 배열을 전달하면 그때는 하나의 배열로 이뤄진 스트림이 생성되는 것이 아니라, "배열에 저장된 요소들로 이뤄진 스트림이 생성"된다.
