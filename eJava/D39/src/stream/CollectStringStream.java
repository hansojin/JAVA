...
/*
 == collect ==
앞서 언급했듯이 한번 파이프라인에 흘려보낸 스트림은 되돌리거나 다른 파이프라인에 
다시 흘려보낼수 없다. 때문에 필요하다면 파이프라인을 통해서 가공되고 걸러진 데이터를 
최종 연산 과정에서 별도로 저장해야 한다. 그리고 이때 다음 메소드를 호출하게 된다.
...
매개변수 선언을 보면 복잡해 보이지만 사용하기는 어렵지 않다. 그럼 예제를 통해서 collect 메소드의 사용 방법을 보이겠다. 
다음은 문자열로 구성된 스트림에서 길이가 5미만인 문자열을 걸러내어 컬렉션 인스턴스에 저장하는 예이다.
...
위의 문장에서 첫 번째 매개변수로 전달된 람다식은 다음과 같다. collect 메소드는 
이 람다식을 기반으로 데이터를 저장할 저장소를 생성한다.
...
사용되지 않는다. 그렇다고 해서 nul을 전달하면 NullPointerException 예외가 발생하니 이어서
