...
class FlatMapStream {
	public static void main(String[] args) {
		...
		System.out.println();	//MY	AGE	YOUR	LIFE	

		
		Stream<String> ss3 = ss1.map(s -> s.split("_"))
				.flatMap(Arrays::stream);	// 자료형이 Stream<String> 이니까 flatMap처리
		ss3.forEach(System.out::println);

		ss1.map(s -> s.split("_"))
			.flatMap(Arrays::stream).forEach(System.out::println);	
		// forEach로 syso 하려면 String이니까 Stream<String> 을 앞에 쓸 수 없지
...
Function<T, R>의 추상 메소드는 R apply(T t)이므로, 위 메소드 호출 시 전달해야 할 
람다식이 구현해야 할 메소드는 다음과 같다.
...
위 예제에서 호출한 String 클래스의 다음 메소드는 인자로 전달된 '기준'을 근거로 문자열을 나누고, 
나뉜 문자열을 배열에 담아서 반환한다.
...
이전에 보인 map은 일대 일로 매핑을 진행하였다. 그러나 위 예제에서 보이듯이 flatMap은 
일대 다의 매핑을 진행한다.(두 개의 긴 문자열을 대상으로 매핑을 진행해서 네 개의 작은 문자열이 되게 하였다.)
 */