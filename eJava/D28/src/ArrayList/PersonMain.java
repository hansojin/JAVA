...
이 문장에서 ArrayList<E>형 참조변수가 아닌 List<E>형 참조변수를 선언한 이유는 코드에 유연성을 제공하기 위함이다. 
주로 List<E>에 선언된 메소드를 호출하기 때문에 굳이 ArrayList<E>형 참조변수를 선언할 필요가 없으며, 
이렇듯 List<E>형 참조변수로 인스턴스를 참조할 경우 다음과 같이 컬렉션 클래스의 교체가 용이해진다.
...
단 배열의 길이를 늘린다는 것은 더 긴 배열로의 교체를 의미한다.(한번 생성된 배열은 길이를 늘릴 수 없으므로) 
따라서 성능에 신경을 써야 한다면 ArrayList<E>의 다음 생성자 정도는 알아 둘 필요가 있다.
...
public ArrayList()
	-> 10개의 인스턴스를 저장할 수 있는 공간을 미리 확보 (default)
