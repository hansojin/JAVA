...
// 이미 정렬기준이 있는데 새롭게 내가 정의하고 싶으면 -> Comparator interface 사용
...
그리고 이를 위해서 "ComparableMyString.java" 소스에서 MyString이라는 String의 Wrapper 클래스를 정의했는데, 
TreeSet<E>의 정렬 기준을 변경하기 위해서 MyString이라는 별도의 클래스를 정의한다는 것이,
사실 이치에는 맞지 않는다. 오히려 다음과 같이 요구할 수 있어야 정상 아니겠는가?

"야!TreeSet<String> 인스턴스! 사전편찬 순서 말고, 길이 순으로 문자열을 졍렬해라!"

이러한 유형의 요구를 위해 정의된 것이 Comparator<T>인터페이스이다. 이 인터페이스는 다음과 같이 정의되어 있다.
...
obj1이 크면 양수를, obj2가 크면 음수를, obj1과 obj2가 같으면 0을 반환하면 된다. 물론 크고 작음에 대한 기준은 여러분이 결정할 몫이다.
*/