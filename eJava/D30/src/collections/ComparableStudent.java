...
class ComparableStudent {
	public static void main(String[] args) {
		...
		tree.add(new Student("YOON", 37));
		tree.add(new Student("HONG", 53));
		tree.add(new Student("PARK", 22));
		tree.add(new Student("LEE", 22));
...
예를 들어서 크고 작음에 대한 기준을 나이로 둘 수 있다. 또는 이름의 가나다 순이 기준이 될 수도 있다. 
즉 크고 작음에 대한 기준은 프로그래머가 결정할 일이다. 
그래서 위와 같이 클래스를 정의할 때에는 다음 인터페이스의 구현을 통해서 크고 작음에 대한 기준을 정해주어야 한다.
...
그러면 인스턴스는 compareTo 메소드의 호출 결과를 바탕으로, 저장된 인스턴스들이 정렬된 상태를 유지하게 한다.
 */
