...
정확히는 다음과 같이 Collection<E>가 Iterable<T>를 상속하는데, ArrayList<E>, LinkedList<E> 클래스는 Collection<E> 인터페이스를 구현하고 있다.
...
이 메소드는 '반복자(Iterator)'라는 것을 반환한다. 
반복자는 저장된 인스턴스들을 순차적으로 참조할 때 사용하는 인스턴스로, 일종의 '지팡이'에 비유할 수 있다. 
그리고 이 지팡이를 얻는 방법은 다음과 같다.(물론 이 지팡이의 역할은 저장된 인스턴스들을 가리키는 것이다.)
...
반복자는 next를 호출할 때마다 첫 번째 인스턴스를 시작으로 다음 인스턴스의 참조 값을 차례로 반환한다. 
그리고 더 이상 반환할 대상이 없을 때 noSuchElementException 예외를 발생시킨다. 따라서 저장된 인스턴스에 차례로 접근할 때에는 다음과 같은 반복문을 구성해야 한다.
...
for-each문을 통한 순차적 접근과 달리 반복자를 이용하면 반복 중간에 특정 인스턴슬르 삭제하는 것이 가능하다. (이는 for-each문을 통해서는 불가능한 일이다.)
...
이러한 반복자는 생성과 동시에 첫 번째 인스턴스를 가리키고, next가 호출될 때마다 가리키는 대상이 다음 인스턴스로 옮겨진다. 
그렇다면 이 반복자를 원하는 때에 다시 첫 번째 인스턴스를 가리키게 하려면 어떻게 해야 할까? 가리키던 위치를 되돌리는 방법은 없으니 다음과 같이 반복자를 다시 얻어야 한다.
