...
src에 전달되는 컬렉션 인스턴스의 저장 공간보다 크거나 최소한 같아야 한다는 점이다. 
만약에 dest로 전달된 인스턴스의 저장 공간이 작다면 복사의 과정에서 공간이 자동으로 늘지 않고 예외가 발생한다.

위 예제에서 보이듯이 컬렉션 인스턴스를 생성하지 않은 상태에서 복사본을 만들려면 다음 방법을 사용하면 된다.

List<String> dest = new ArrayList<>(src);

즉 copy 메소드는 위의 문장을 대신하지 않는다. 
그러나 이미 생성된 컬렉션 인스턴스의 내용을 통째로 바꾸려는 경우에 copy 메소드는 유용하게 사용된다. 
*/